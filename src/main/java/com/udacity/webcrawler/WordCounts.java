...
//    here wordCounts.entrySet() returns a set view of entire map by using getKey() and getValue() method we can find corresponding key and value
//    by using .stream I am creating a stream from the collection
//    .sorted method will sort the stream
//    WordCountComparator class is implementing Comparable<Map.Entry<String, Integer>> interface and compareTo method has overrided
//    finally I am collecting the stream by using .collect and converting it into map by using Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (key, value) -> key, LinkedHashMap::new)
